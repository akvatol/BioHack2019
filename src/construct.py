...
class APGraph(nx.DiGraph):
    def all_vs_all_paths(self, out_file, decode_dic, custom=False):
        ...
        for i, p1 in enumerate(self.my_tpsort[:-1]):
            self.my_ordered_pairs += [(p1, p2)
                                      for p2 in self.my_tpsort[i + 1:]]
...
class Parse_to_Graph:
    def __init__(self, path: str, path_to_db: str, db_wkeys):
        self.G = APGraph()
        self.file = path
        db = h5py.File(path_to_db, 'r')
    ...
    def line_parser(self, string: str):
        ...
        if int(pair[0:len(pair) // 2]) > int(pair[len(pair) // 2:]):
            self.G.add_edge(
                self.indices[node1], self.indices[node2], weight=weight)
        else:
            self.G.add_edge(
                self.indices[node2], self.indices[node1], weight=weight)
    ...
    def longest_peptide(self):
        if self.flag and self.is_acyclic:
            ...
            for i in range(0, len(lpath) - 1, 2):
                ...
                lpep += (self.inv_indices[lpath[i]] +
                         self.inv_indices[lpath[i + 1]][weight:])
            return lpep
        else:
            raise ValueError(
                "Graph haven't been yet constructed or it contains cycles")
...
def getArgs():
    ...
    # parser.add_argument('njobs', '--number-of-cores', type=str,
